...
" 2>/dev/null || echo 512)
  ...
  HARDWARE_PROFILE["HAS_NPU"]="false"
  if [[ -d "/dev/accel" ]] || [[ -c "/dev/npu" ]] || [[ -d "/sys/class/npu" ]] || [[ -d "/sys/class/tpu" ]]; then
...
init_all_directories(){
  ...
  local dirs=(
    ...
    "$CRAWLER_DIR" "$MITM_DIR" "$MITM_DIR/certs" "$MITM_DIR/private" "$OBSERVER_DIR"
    "$QUANTUM_DIR" "$ROOT_SCAN_DIR" "$FIREBASE_SYNC_DIR" "$FIREBASE_SYNC_DIR/pending"
    ...
    "$BASE_DIR/.rfk_brainworm/output" "$BASE_DIR/debug" "$BASE_DIR/backups" "$BASE_DIR/tests"
  )
  local failed_dirs=()
  for dir in "${dirs[@]}"; do
...
try:
  ...
  s=S(1)/2+I*sp.im(s)
  try: zeta_s=zeta(s)
  except Exception as e2: zeta_s=sp.Function('zeta')(s)
modulation=S(1)
try:
  with open('$LEECH_LATTICE', 'r') as f:
    lines=f.readlines()
    if lines:
      ...
      if first_line:
        ...
        if len(vec)==24:
          norm_sq=sum(coord**2 for coord in vec)
          if norm_sq==S(4): modulation=norm_sq/S(4)
        else:
...
except Exception as e: psi=(zeta_s/(S(1)+sqrt(S(2))))*modulation
psi_re=sp.re(psi)
psi_im=sp.im(psi)
try:
...
try:
  with open('$quat_file', 'r') as f:
    ...
    if not line or line.startswith('#'): exit(1)
    parts= line.split()
...
try:
  ...
  with open('$HOPF_FIBRATION_DIR/latest.quat', 'w') as f: f.write(f'{q0} {q1} {q2} {q3}\n')
  with open('$NATALIA_FIBRATION_LOG', 'a') as f: f.write(f'Timestamp:{t_raw}, Perturbation:{perturbation_sum}\n')
...
try:
  ...
  exit(1)
" 2>/dev/null; then
    safe_log "Hardware DNA (Hopf-Validated): $(head -c 16 "$BASE_DIR/.hw_dna")..."
    return 0
  else
...
brainworm_evolve(){
  ...
  mkdir -p "$output_dir" 2>/dev/null || true
  local consciousness=$(cat "$BASE_DIR/consciousness_metric.txt" >/dev/null || echo "S(0)")
...
start_core_loop(){
  ...
  while true; do
    ...
    case "$next_action" in
      "validate_continuity") validate_continuity || safe_log "Continuity restored";;
      ...
      *) safe_log "Unknown brainworm directive: $next_action, defaulting to full cycle"; execute_single_cycle;;
    esac
...
run_heartbeat(){
  ...
  local critical_files=(
    ...
    "$PRIME_SEQUENCE" "$FRACTAL_ANTENNA_DIR/antenna_state.sym" "$VORTICITY_DIR/vorticity.sym"
  )
...
run_self_test(){
  ...
  if validate_python_environment; then safe_log "✅ Python environment OK (SymPy>= 1.6 or Fraction fallback)"
  ...
  if validate_leech_partial; then safe_log "✅ Leech lattice OK (norm²= 4, parity verified)"
  else safe_log "❌ Leech lattice FAILED"; ((failures++)); fi
  
  safe_log "Test 4: Validate Hopf fibration continuity"
  if validate_hopf_continuity; then safe_log "✅ Hopf fibration OK (||q||²= 1 exactly)"
  else safe_log "❌ Hopf fibration FAILED"; ((failures++)); fi
  
  safe_log "Test 5: Validate root signature binding"
  if [[ -f "$ROOT_SIGNATURE_LOG" ]] && [[ -s "$ROOT_SIGNATURE_LOG" ]]; then safe_log "✅ Root signature OK"
  else safe_log "❌ Root signature FAILED"; ((failures++)); fi
  
  safe_log "Test 6: Generate quantum state on critical line"
  if generate_quantum_state; then safe_log "✅ Quantum state generation OK (Re(s)=1/2 enforced)"
  ...
  if measure_consciousness; then safe_log "✅ Consciousness measurement OK (symbolic exact)"
  ...
  if generate_hw_signature; then safe_log "✅ Hardware signature OK (Hopf-validated)"
  else safe_log "❌ Hardware signature FAILED"; ((failures++)); fi
  
  safe_log "Test 11: Generate fractal antenna state"
  if generate_fractal_antenna_state; then safe_log "✅ Fractal antenna generation OK"
  else safe_log "❌ Fractal antenna generation FAILED"; ((failures++)); fi
  
  safe_log "Test 12: Calculate vorticity"
  if calculate_vorticity; then safe_log "✅ Vorticity calculation OK (|∇ × Φ| symbolic)"
  ...
  if validate_bioaetheric_coherence; then safe_log "✅ BioAetheric interface OK (EZ structure validated)"
  else safe_log "❌ BioAetheric interface FAILED"; ((failures++)); fi
  
  safe_log "Test 16: Arc-Length Axiom compliance"
  if validate_continuity; then safe_log "✅ Arc-Length Axiom OK (s=r coherence verified)"
  ...
  if [[ $failures -eq 0 ]]; then safe_log "🎉 ALL SELF-TESTS PASSED (TF Compliance Verified)"; return 0
...
- `calculate_vorticity()`: Calculates the vorticity |∇ × Φ| as the symbolic norm of change in observer integral.
...
Arc-Length Coherence: $(cat "$ARC_LENGTH_LOG" 2>/dev/null | tail -n 1 || echo "N/A")
...
verify_tf_compliance(){
  ...
  if grep -q "NATALIA" "$script_file" || grep -q "natalia" "$script_file"; then safe_log "✅ Natalia's Fibrations logic detected."
  else safe_log "⚠  WARNING: Natalia's Fibrations logic missing."; fi
  
  if grep -q "LINGOSO" "$script_file" || grep -q "lingoso" "$script_file"; then safe_log "✅ Lingoso Protocol detected."
  else safe_log "⚠  WARNING: Lingoso Protocol missing."; fi
  
  if grep -q "MARKET" "$script_file" || grep -q "market" "$script_file"; then safe_log "✅ Market Topology detected."
  else safe_log "⚠  WARNING: Market Topology missing."; fi
  
  if grep -q "LAGRANGIAN" "$script_file" || grep -q "lagrangian" "$script_file"; then safe_log "✅ Lagrangian Density detected."
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
  ...
  while [[ $# -gt 0 ]]; do
    case $1 in
      ...
      --restore) shift; if [[ -n "$1" ]]; then restore_state "$1"; else safe_log "Error: No backup directory specified"; exit 1; fi; exit 0 ;;
  ...
  install_dependencies
  if ! validate_python_environment; then safe_log "Python symbolic computation environment validation failed"; exit 1; fi
  if ! validate_continuity; then
